...
<div class="ui small breadcrumb">
  ...
  <i class="right chevron icon divider"></i>
  <div class="active section">编辑作品</div>
</div>
</div>
<div class="container" ng-app="newWorkApp" ng-controller="newWorkCtrl">
  <form action="/admin/api/work/upLoadPic" enctype="multipart/form-data" method="post" class="ui form" id="uploadPicForm">
    ...
    <img id='previewImag' src="/Public/WorkImages/<?php echo ($data['cover']); ?>" alt="" />
    </div>
    <div class="field">
      <label>更新作品图片</label>
  ...
  <div class="ui form">
      <input type="hidden" name="cover" id="coverStringHidden"  value="<?php echo ($data['cover']); ?>" />
      <input type="hidden" name="thumbnail" id="thumbnailStringHidden"  value="<?php echo ($data['thumbnail']); ?>"  />
      <input type="hidden" name="wid" id="work_wid" value="<?php echo ($data['wid']); ?>">
    <div class="field">
      <label>作品名称</label>
      <input id="work_title" type="text" value="<?php echo ($data['title']); ?>" placeholder="<?php echo ($data['title']); ?>">
    </div>
    <div class="field">
      <label>作品描述</label>
      <textarea id="work_content" rows="2"  placeholder="<?php echo ($data['content']); ?>"><?php echo ($data['content']); ?></textarea>
    </div>
    <div class="field">
      <label>作者名字</label>
      <input id="work_author" type="text" value="<?php echo ($data['author']); ?>" placeholder="<?php echo ($data['author']); ?>" >
    </div>
    <div class="field">
      <input type="button" ng-click="createNewWork()" class="ui button default" value="提交">
...
<script src="/Public/admin/js/modules/edit_work.js"></script>
