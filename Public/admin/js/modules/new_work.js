...
newWorkApp.controller('newWorkCtrl', function($scope, $http){
  $scope.createNewWork = function(cover, thumbnail, title, content, author){
      console.log(cover+"###"+thumbnail+"###"+title+"###"+content+"###"+author);
      $.post('/admin/api/work/create', { cover: $("#coverStringHidden").val(), thumbnail: $("#thumbnailStringHidden").val(), title: title, content: content, author: author }, function (response) {
        loader.modal("show", function(){
            if (response != null | response != undefined) {
              window.location.assign('/admin/page/work?massage=作品创建成功！');
            }
        });
      });
  };
